...
namespace Configs {
    bool uTLS::ParseFromLink(const QString& link)
    {
        ...
        {
            enabled = true;
            fingerPrint = query.queryItemValue("fp");
        }
        return true;
    }
    ...
    bool TLS::ParseFromLink(const QString& link)
    {
        ...
        if (query.hasQueryItem("disable_sni")) disable_sni = query.queryItemValue("disable_sni") == "true";
        if (query.hasQueryItem("sni")) server_name = query.queryItemValue("sni");
        if (query.hasQueryItem("allowInsecure")) insecure = query.queryItemValue("allowInsecure") == "true";
