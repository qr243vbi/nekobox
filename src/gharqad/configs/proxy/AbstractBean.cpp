...
QString AbstractBean::ToNekorayShareLink() const {
  ...
  url.setHost(type());
  url.setFragment(QJsonObject2QString(b, true).toUtf8().toBase64(
      QByteArray::Base64UrlEncoding));
  return url.toString();
}

int AbstractBean::Id() const { return this->entity->id; }

QString AbstractBean::type() const { return "unknown"; }
...
bool AbstractBean::TryParseYaml(const Configs::Data::Node &obj){
    return this->TryParseJson(obj);
};
...
void AbstractBean::ResolveDomainToIP(const std::function<void()> &onFinished) {
  ...
  bool noResolve = dynamic_cast<ChainBean *>(this) != nullptr ||
  ...
  QThreadPool::globalInstance()->start([this, serverAddress, onFinished] {
    QString ip;
    ...
    if (getaddrinfo(serverAddress.toUtf8().constData(), nullptr, &hints,
                    &result) == 0) {
    ...
    //        [this, serverAddress, ip, onFinished]
    {
      if (!ip.isEmpty()) {
        ...
        if (stream) {
          ...
          if (stream->network->toString().compare("ws", Qt::CaseInsensitive) ==
                  0 &&
              stream->host.isEmpty()) {
...
bool AbstractBean::TryParseNekorayLink(const QUrl &link) {
  ...
  if (ret) {
    ...
    this->entity->serverAddress =
        json["addr"].toString(this->entity->serverAddress);
    this->entity->serverPort = json["port"].toInt(this->entity->serverPort);
  }
  return ret;
}

CoreObjOutboundBuildResult AbstractBean::BuildCoreObjSingBox() const {
  return {};
};

QString AbstractBean::ToShareLink() const {
  return this->ToNekorayShareLink();
};

bool AbstractBean::IsEndpoint() const { return false; };

bool AbstractBean::TryParseLink(const QString &link) {
  return this->TryParseNekorayLink(link);
};

bool AbstractBean::TryParseJson(const Configs::Data::Node &obj) {
  return false;
};
} // namespace Configs
